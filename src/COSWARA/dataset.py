...
class Multimodal_Datasets(Dataset):
    def __init__(self, dataset_path, data='Audformer', split_type='train'):
        ...
        self.m22 = torch.tensor(dataset[split_type]['vowel_mfcc'].astype(np.float32)).cpu().detach()
        self.m23 = torch.tensor(dataset[split_type]['vowel_rms'].astype(np.float32)).cpu().detach()
        self.m24 = torch.tensor(dataset[split_type]['vowel_zcr'].astype(np.float32)).cpu().detach()
        self.m25 = torch.tensor(dataset[split_type]['vowel_cenrtoid'].astype(np.float32)).cpu().detach()
        self.m26 = torch.tensor(dataset[split_type]['vowel_log_mel'].astype(np.float32)).cpu().detach()
        self.m27 = torch.tensor(dataset[split_type]['vowel_gfcc'].astype(np.float32)).cpu().detach()
        self.m28 = torch.tensor(dataset[split_type]['vowel_cqcc'].astype(np.float32)).cpu().detach()
